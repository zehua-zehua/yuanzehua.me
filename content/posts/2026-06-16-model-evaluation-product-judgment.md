---
title: 模型评测不是跑分，而是定义什么叫可用
date: 2026-06-16
tag: AI
lang: zh
summary: 从图像编辑场景里的 bad case 归因出发，思考模型评测如何从 benchmark 跑分走向产品可用性的判断体系。
---

# 模型评测不是跑分，<br>而是定义什么叫可用
...
站在产品视角看，模型评测和 AB Test 在本质上有一些重合之处。

AB Test 不是为了得到一个漂亮数字，而是为了支持产品决策。它需要假设、样本、指标、分组、置信度和解释框架。模型评测也是这样。它同样需要先明确要验证什么能力，选什么样本，怎么分层，观察哪些指标，怎样处理主观偏好，最后如何把结果转化成“是否上线”“上线到哪里”“还需要补什么”的决策。

两者都在处理不确定性。

AB Test 处理的是用户行为的不确定性，模型评测处理的是模型输出的不确定性。前者问的是：用户会不会因为这个改动产生更好的行为？后者问的是：模型会不会在这个任务里稳定地产生可接受的结果？

更进一步看，模型评测可以被理解成 AI 产品里的前置实验系统。

在真正放量之前，团队需要先通过离线评测、专家盲测、小规模灰度和线上反馈，逐步建立对模型能力的信心。这个过程和 AB Test 一样，不是一次性动作，而是一套持续迭代的实验机制。

模型上线之后，评测也不应该结束。

用户输入会变化，模型版本会变化，数据分布会变化，产品定位也会变化。一个在今天可用的能力，未来可能因为场景扩展而暴露新的问题。评测体系如果不能持续更新，就会很快变成过期的安全感。
